...
namespace App\Services\PurchaseStore;
 ...
 * @package App\Services\PurchaseStore
 */
class PurchaseStoreService extends BaseService
{
    protected $model;
    ...
    public function getById(int $id): array
    {
        ...
        if (!empty($ret)) {
            ...
            $detail['source_id'] = data_get($ret, 'source_id');
            ...
            if (!empty($detailIds)) {
                ...
                if (!empty($relations)) {
                    ...
                    $snInfo = $this->snCode->whereIn('id', $snIds)->get(['id', 'box', 'sn', 'goods_no'])
                        ->groupBy(['goods_no', 'box'])->toArray();
                }
            }
            foreach (data_get($ret, 'detail') as $v) {
                ...
                $item['existing_num'] = data_get($storage, $goods_no, 0);
                $item['usable_num'] = data_get($storage, $goods_no, 0)
                    - data_get($lockNum, $goods_no, 0);
