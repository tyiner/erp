...
class MoneyLogService extends BaseService
{
    /**
     * 修改用户金额 & 增加日志
     * @param $name 订单支付|在线充值
     * @param $user_id
     * @param $money
     * @param int $type 0金额 1冻结资金 2积分
     * @param string $info
     * @return array
     */
    public function editMoney($name, $user_id, $money, $type = 0, $info = '')
    {
        try {
            ...
            $moneyLog->user_id = $user_id;
            $moneyLog->money = $money;
            $moneyLog->is_type = $type;
            $moneyLog->name = $name;
            $moneyLog->info = $info;
            $moneyLog->save();
            ...
            switch ($type) {
                case 0:
                    ...
                    break;
                case 1:
                    $member->frozen_money += $money;
                    break;
                case 2:
                    $member->integral += $money;
                    break;
            }
            $member->save();
            DB::commit();
            ...
            DB::rollBack();
            return $this->format_error('资金日志修改失败' . $e->getMessage());
        }
    }
