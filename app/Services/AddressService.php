...
class AddressService extends BaseService
{
    ...
    public function get($id) {
        if (empty($id)) {
            throw new \Exception('address_id不能为空', 4510);
        }

        $address = Address::find($id);
        if (empty($address)) {
            throw new \Exception('收货地址不存在#'.$id, 4511);
        }

        if ($this->getUserId() != $address->user_id) {
            throw new \Exception('非法获取收货地址', 4512);
        }
...
//        $address_model->area_info = $area_info[0]['name'].' '.$area_info[1]['name'].' '.$area_info[2]['name'];
        $address_model->area_info = request()->area_info ?? '';
