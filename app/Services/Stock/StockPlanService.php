...
class StockPlanService extends BaseService
{
    ...
    public function getDetailList(array $data): LengthAwarePaginator
    {
        ...
        if (!is_null($ret->items())) {
            ...
            $purchaseIds = $ret->pluck('id')->unique()->toArray();
            $shipType = PURCHASE_STOCK_SHIP;
            $shipInfos = $this->getStoreInfo($purchaseIds, $shipType);
            $outType = PURCHASE_STOCK_RED;
            $backInfos = $this->getStoreInfo($purchaseIds, $outType);
            foreach ($ret->items() as &$item) {
                $goodsNo = data_get($item, 'goods_no');
                $goods = $this->getGoodsByNo($goodsNo);
                $supplier = $this->getSupplier(data_get($item, 'supplier_id'));
                $locationId = data_get($item, 'location_id');
                $shipNum = data_get($shipInfos, data_get($item, 'id') . '.' . $goodsNo, 0);
                $backNum = data_get($backInfos, data_get($item, 'id') . '.' . $goodsNo, 0);
                $usableNum = data_get($storageNum, $goodsNo, 0) - data_get($lockNum, $goodsNo, 0);
                data_set($item, 'usable_num', $usableNum);
                data_set($item, 'ship_num', $shipNum);
                data_set($item, 'back_num', $backNum);
     ...
     * @return array
     */
    private function getStoreInfo(array $purchaseOrderIds, $type = PURCHASE_STOCK_RED): array
    {
        $query = $this->purchaseDetail
            ->whereIn('source_id', $purchaseOrderIds)
            ->where('type', $type);
        if (PURCHASE_STOCK_RED == $type) {
