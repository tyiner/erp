...
class StoreController extends Controller
{
    ...
    // 获取店铺信息
    public function show(StoreService $store_service, GoodsService $goods_service, $id)
    {
    ...
    // 获取店铺商品列表
    public function getGoodsList(GoodsService $goods_service, $id)
    {
        $info = $goods_service->getStoreGoodsList($id);
        if ($info['status']) {
            return $this->success($info['data']);
        } else {
            return $this->error($info['msg']);
     ...
     * @return array
     */
    public function getGoodsInfo(StoreService $service, $id, $goods_id)
    {
        try {
            $info = $service->getGoodsInfo($id, $goods_id);
    ...
    // 搜索店铺的商品
    public function goodsSearch(GoodsService $service, $id)
    {
        $info = $service->goodsSearch($id);
        if ($info['status']) {
            return $this->success($info['data']);
        } else {
            return $this->error($info['msg']);
    ...
    public function getGoodsStock($id, $goods_id) {
        $storeGoods = StoreGoods::where('store_id', $id)
            ->whereIn('goods_id', explode(',', $goods_id))
            ->select('goods_id', 'goods_stock')
            ->get();
        return $this->success($storeGoods);
    }
}
